								  ...
								  IWallMarkArray& wallmarks_vector,int t)
{
	...
	if(pMaterial->Flags.is(SGameMtl::flBloodmark))
	{
		...
		{		
            EnvCryRay.Render->add_StaticWallmark(&wallmarks_vector, end_point, wallmark_size, pTri, pVerts);
		}
...
void CWalmarkManager::StartWorkflow()
{
	...
	for (CDB::RESULT* Res=R_begin; Res!=R_end; ++Res)
	{
		...
		{
            EnvCryRay.Render->add_StaticWallmark(&*m_wallmarks, end_point, m_wallmark_size, _t, V_array);
			++wm_count;
		}else
			++_not_dist;
