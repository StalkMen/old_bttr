...
void ui_core::PushScissor(const Frect& r_tgt, bool overlapped)
{
	...
	EnvCryRay.UIRender->SetScissor(&r);
}
...
void ui_core::PopScissor()
{
	...
	if(m_Scissors.empty())
		EnvCryRay.UIRender->SetScissor(NULL);
	else{
		...
		EnvCryRay.UIRender->SetScissor(&tgt);
	}
}
...
void ui_core::pp_start()
{
	...
	m_pp_scale_.set	( float(EnvCryRay.Render->getTarget()->get_width())/float(UI_BASE_WIDTH),	float(EnvCryRay.Render->getTarget()->get_height())/float(UI_BASE_HEIGHT) );
	m_2DFrustumPP.CreateFromRect(Frect().set(	0.0f,
												0.0f,
												float(EnvCryRay.Render->getTarget()->get_width()),
												float(EnvCryRay.Render->getTarget()->get_height())
												));

	m_current_scale			= &m_pp_scale_;
	
	g_current_font_scale.set(	float(EnvCryRay.Render->getTarget()->get_width())/float(Device.dwWidth),
								float(EnvCryRay.Render->getTarget()->get_height())/float(Device.dwHeight) );
