...
CAlienEffectcsPP::CAlienEffectcsPP(const SPPInfo &ppi, EEffectorPPType type) :
...
public:
					CAlienEffectcs	(ECamEffectorType type, CAI_Bloodsuckcs *obj);
...
CAlienEffectcs::CAlienEffectcs(ECamEffectorType type, CAI_Bloodsuckcs *obj) :
...
CBloodsuckcsAlien::CBloodsuckcsAlien()
{
	m_object	= 0;
}
...
void CBloodsuckcsAlien::init_external(CAI_Bloodsuckcs *obj)
{
	m_object	= obj;
}

void CBloodsuckcsAlien::reinit()
{
	m_active				= false;	
	m_crosshair_show		= false;
}

void CBloodsuckcsAlien::activate()
{
	if (m_active) return;
...
void CBloodsuckcsAlien::deactivate()
{
	if (!m_active) return;
