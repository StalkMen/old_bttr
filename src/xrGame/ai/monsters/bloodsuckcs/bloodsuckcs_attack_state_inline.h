...
#define CBloodsuckcsStateAttackAbstract CBloodsuckcsStateAttack<_Object>

template <typename _Object>
CBloodsuckcsStateAttackAbstract::CBloodsuckcsStateAttack(_Object *obj) : inherited_attack(obj)
{
	add_state	(eStateAttack_Hide,	xr_new<CStateMonsterBackstubEnemy<_Object>> (obj));
}

template <typename _Object>
CBloodsuckcsStateAttackAbstract::~CBloodsuckcsStateAttack()
{
}

template <typename _Object>
void CBloodsuckcsStateAttackAbstract::initialize()
{
...
template <typename _Object>
void CBloodsuckcsStateAttackAbstract::finalize()
{
	inherited::finalize();
...
template <typename _Object>
void CBloodsuckcsStateAttackAbstract::critical_finalize()
{
...
{
	const float invisibility_dist2enemy     = 5.f;
...
template <typename _Object>
void CBloodsuckcsStateAttackAbstract::execute()
{
	if ( check_home_point() )
	{
...
template <typename _Object>
void CBloodsuckcsStateAttackAbstract::update_invisibility()
{
		...
		if ( Device.dwTimeGlobal > m_time_stop_invis + object->get_invisibility_activate_delay() )
		{
			...
			if ( current_substate == eStateAttack_Hide )
			{
				object->start_invisible_predator();
				      ...
				      detail::bloodsuckcs::invisibility_dist2enemy )
			{
				object->start_invisible_predator();
			}
		}
...
template <typename _Object>
bool CBloodsuckcsStateAttackAbstract::check_hiding()
{
	const bool health_step_lost = object->conditions().GetHealth() < 
		                          m_last_health-detail::bloodsuckcs::loose_health_diff;
...
template <typename _Object>
void CBloodsuckcsStateAttackAbstract::setup_substates()
{
...
#undef CBloodsuckcsStateAttackAbstract
...
void   CStateMonsterBackstubEnemy<_Object>::initialize ()
{
	...
	m_encircle_end_tick = Device.dwTimeGlobal + detail::bloodsuckcs::encircle_time;
...
void   CStateMonsterBackstubEnemy<_Object>::execute ()
{
	...
	if ( object->conditions().GetHealth() < m_last_health-detail::bloodsuckcs::loose_health_diff &&
		 Device.dwTimeGlobal > m_next_change_behaviour_tick )
	{
		m_next_change_behaviour_tick = Device.dwTimeGlobal + detail::bloodsuckcs::change_behaviour_time;
		m_last_health = object->conditions().GetHealth();
