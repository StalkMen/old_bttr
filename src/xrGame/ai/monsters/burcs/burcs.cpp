...
void CBurcs::net_Destroy()
{
	inherited::net_Destroy();
	TScanner::on_destroy();
}

void CBurcs::reload(LPCSTR section)
{
...
void CBurcs::Load(LPCSTR section)
{
...
void CBurcs::shedule_Update(u32 dt)
{
...
void CBurcs::CheckSpecParams(u32 spec_params)
{
}
...
void CBurcs::StartTeleObjectParticle(CGameObject *pO) 
{
...
void CBurcs::StopTeleObjectParticle(CGameObject *pO) 
{
	CParticlesPlayer* PP = smart_cast<CParticlesPlayer*>(pO);
	if(!PP) return;
...
void CBurcs::on_scanning()
{
	time_last_scan = Device.dwTimeGlobal;
}

void CBurcs::on_scan_success()
{
	CActor *pA = smart_cast<CActor *>(Level().CurrentEntity());
	if (!pA) return;
