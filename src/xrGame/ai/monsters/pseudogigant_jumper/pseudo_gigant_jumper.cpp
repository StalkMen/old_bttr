...
#include "pseudo_gigant_step_effector_jumper.h"
#include "../../../actor.h"
...
CPseudoGigant_jumper::CPseudoGigant_jumper()
{
	...
	StateMan = xr_new<CStateManagerGigant_jumper>(this);
...
void CPseudoGigant_jumper::Load(LPCSTR section)
{
	...
	m_jump_damage			= pSettings->r_float(section, "jump_attack_damage");
...
void CPseudoGigant_jumper::reinit()
{
	inherited::reinit();
...
void CPseudoGigant_jumper::event_on_step()
{
	...
	if(pActor)
	{
		...
		if (dist_to_actor < max_dist) 
			Actor()->Cameras().AddCamEffector(xr_new<CPseudoGigant_jumperStepEffector>(step_effector.time, step_effector.amplitude, step_effector.period_number, (max_dist - dist_to_actor) / (1.2f * max_dist))
			);
	}
}

bool CPseudoGigant_jumper::check_start_conditions(ControlCom::EControlType type)
{
...
void CPseudoGigant_jumper::on_activate_control(ControlCom::EControlType type)
{
	if (type == ControlCom::eControlThreaten) {
...
void CPseudoGigant_jumper::on_threaten_execute()
{
	// разбросить объекты
	m_nearest.clear();
...
void CPseudoGigant_jumper::HitEntityInJump		(const CEntity *pEntity) 
{
	...
	HitEntity			(pEntity, m_jump_damage, params.impulse, params.impulse_dir);

	//oldsSerpski stalker, урон от удара лапой
	on_threaten_execute();
}

void CPseudoGigant_jumper::TranslateActionToPathParams()
{
