...
CAlienEffectsocPP::CAlienEffectsocPP(const SPPInfo &ppi, EEffectorPPType type) :
...
public:
					CAlienEffectsoc (ECamEffectorType type, CAI_Bloodsucksoc *obj);
...
CAlienEffectsoc::CAlienEffectsoc(ECamEffectorType type, CAI_Bloodsucksoc *obj) :
...
CBloodsucksocAlien::CBloodsucksocAlien()
{
	m_object	= 0;
}
...
void CBloodsucksocAlien::init_external(CAI_Bloodsucksoc *obj)
{
	m_object	= obj;
}

void CBloodsucksocAlien::reinit()
{
	m_active				= false;	
	m_crosshair_show		= false;
}

void CBloodsucksocAlien::activate()
{
	if (m_active) return;
...
void CBloodsucksocAlien::deactivate()
{
	if (!m_active) return;
