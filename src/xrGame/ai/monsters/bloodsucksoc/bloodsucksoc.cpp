...
#include "bloodsucksoc_vampire_effector.h"
...
u32 CAI_Bloodsucksoc::m_time_last_vampire = 0;

CAI_Bloodsucksoc::CAI_Bloodsucksoc()
{
	StateMan						= xr_new<CStateManagerBloodsucksoc>(this);
...
void CAI_Bloodsucksoc::Load(LPCSTR section) 
{
...
void CAI_Bloodsucksoc::reload(LPCSTR section)
{
...
void CAI_Bloodsucksoc::LoadVampirePPEffector(LPCSTR section)
{
...
void CAI_Bloodsucksoc::SatisfyVampire()
{
	m_vampire_want_value = 0.f;
...
void  CAI_Bloodsucksoc::BoneCallback(CBoneInstance *B)
{
	CAI_Bloodsucksoc*	this_class = static_cast<CAI_Bloodsucksoc*> (B->callback_param());

	this_class->Bones.Update(B, Device.dwTimeGlobal);
}

void CAI_Bloodsucksoc::vfAssignBones()
{
...
void CAI_Bloodsucksoc::CheckSpecParams(u32 spec_params)
{
...
BOOL CAI_Bloodsucksoc::net_Spawn (CSE_Abstract* DC) 
{
...
void CAI_Bloodsucksoc::UpdateCL()
{
	inherited::UpdateCL				();
...
bool CAI_Bloodsucksoc::done_enough_hits_before_vampire()
{
	return (int)m_hits_before_vampire >=
		(int)m_sufficient_hits_before_vampire + m_sufficient_hits_before_vampire_random;
}

void CAI_Bloodsucksoc::on_attack_on_run_hit() { ++m_hits_before_vampire; }

void CAI_Bloodsucksoc::shedule_Update(u32 dt)
{
...
void CAI_Bloodsucksoc::Die(CObject* who)
{
	predator_stop				();
	inherited::Die				(who);
}

void CAI_Bloodsucksoc::post_fsm_update()
{
	inherited::post_fsm_update();
}

bool CAI_Bloodsucksoc::check_start_conditions(ControlCom::EControlType type)
{
...
void CAI_Bloodsucksoc::set_alien_control(bool val)
{
	val ? m_alien_control.activate() : m_alien_control.deactivate();
}

void CAI_Bloodsucksoc::predator_start()
{
	...
	sound().play					(CAI_Bloodsucksoc::eChangeVisibility);

	m_predator						= true;
	state_invisible					= false;
}

void CAI_Bloodsucksoc::predator_stop()
{
	if (!m_predator)				return;
...
void CAI_Bloodsucksoc::predator_freeze()
{
	control().animation().freeze	();
}

void CAI_Bloodsucksoc::predator_unfreeze()
{
	control().animation().unfreeze();
}

void CAI_Bloodsucksoc::move_actor_cam()
{
...
void CAI_Bloodsucksoc::HitEntity(const CEntity *pEntity, float fDamage, float impulse, Fvector &dir)
{
...
void CAI_Bloodsucksoc::start_invisible_predator()
{
	state_invisible				= true;
	predator_start				();
}
void CAI_Bloodsucksoc::stop_invisible_predator()
{
	state_invisible				= false;
	predator_stop				();
}

void CAI_Bloodsucksoc::manual_activate()
{
	state_invisible = true;
	setVisible		(FALSE);
}

void CAI_Bloodsucksoc::manual_deactivate()
{
	state_invisible = false;
	setVisible		(TRUE);
}

void CAI_Bloodsucksoc::on_activate_control(ControlCom::EControlType type)
{
	if (type == ControlCom::eControlThreaten) {
