...
void CActor::UpdateCL()
{
    ...
    if (pWeapon)
    {
        ...
        if (Level().CurrentEntity() && this->ID() == Level().CurrentEntity()->ID())
        {
			...
			g_pGamePersistent->m_pGShaderConstants->hud_params.w = pWeapon->GetZRotatingFactor(); //--#SM+#--
...
void CActor::UpdateArtefactsOnBeltAndOutfit()
{
        ...
        inventory().m_belt.end() != it; ++it)
    {
        ...
        if (artefact)
        {
            ...
            fArtReac += artefact->m_fArtefReactionRestoreSpeed * art_cond;
            conditions().ChangeArtefReaction(fArtReac);
...
//										    }
        ...
        case ALife::eKurrRestoreSpeed:
        {
                                            res = conditions().V_Kurr();
                                            break;
        }
    }//switch
