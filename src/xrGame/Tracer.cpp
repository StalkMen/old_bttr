...
IC void FillSprite_Circle      (const Fvector& pos, const float width, const float length, u32 color)
{
	...
	//	Tri 1
	EnvCryRay.UIRender->PushPoint(d.x+pos.x, d.y+pos.y, d.z+pos.z, color, t_crcl.min.x, t_crcl.max.y);		// 0.f,1.f);
	EnvCryRay.UIRender->PushPoint(a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_crcl.min.x, t_crcl.min.y);		// 0.f,0.f);
	EnvCryRay.UIRender->PushPoint(c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_crcl.max.x, t_crcl.max.y);		// 1.f,1.f);
	//	Tri 2																					
	EnvCryRay.UIRender->PushPoint(c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_crcl.max.x, t_crcl.max.y);	// 1.f,1.f);
	EnvCryRay.UIRender->PushPoint(a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_crcl.min.x, t_crcl.min.y);	// 0.f,0.f);
	EnvCryRay.UIRender->PushPoint(b.x+pos.x, b.y+pos.y, b.z+pos.z, color, t_crcl.max.x, t_crcl.min.y);	// 1.f,0.f);
...
IC void FillSprite_Line	(const Fvector& pos, const Fvector& dir, const float width, const float length, u32 color)
{
	...
	//	TODO: return code back to indexed rendering since we use quads
	//	Tri 1
	EnvCryRay.UIRender->PushPoint(d.x+pos.x, d.y+pos.y, d.z+pos.z, color, t_tracer.min.x, t_tracer.max.y);
	EnvCryRay.UIRender->PushPoint(a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_tracer.min.x, t_tracer.min.y);
	EnvCryRay.UIRender->PushPoint(c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_tracer.max.x, t_tracer.max.y);
	//	Tri 2																					
	EnvCryRay.UIRender->PushPoint(c.x+pos.x, c.y+pos.y, c.z+pos.z, color, t_tracer.max.x, t_tracer.max.y);
	EnvCryRay.UIRender->PushPoint(a.x+pos.x, a.y+pos.y, a.z+pos.z, color, t_tracer.min.x, t_tracer.min.y);
	EnvCryRay.UIRender->PushPoint(b.x+pos.x, b.y+pos.y, b.z+pos.z, color, t_tracer.max.x, t_tracer.min.y);
...
void  CTracer::Render	(const Fvector& pos, 
						 ...
						 bool bActor)
{
	if (EnvCryRay.Render->ViewBase.testSphere_dirty((Fvector&)center,length*.5f) )
	{
