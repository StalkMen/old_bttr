...
{
	EnvCryRay.Render->set_Transform		(&m_item_transform);
	EnvCryRay.Render->add_Visual		(m_model->dcast_RenderVisual());
	debug_draw_firedeps			();
...
attachable_hud_item::~attachable_hud_item()
{
	...
	EnvCryRay.Render->model_Delete		(v);
	m_model						= NULL;
}
...
void attachable_hud_item::load(const shared_str& sect_name)
{
	...
	m_model						 = smart_cast<IKinematics*>(EnvCryRay.Render->model_Create(visual_name.c_str()));
...
void player_hud::load(const shared_str& player_hud_sect)
{
	...
	bool b_reload = (m_model != NULL || m_model_2 != NULL);
	if(m_model)
	{
		IRenderVisual* v			= m_model->dcast_RenderVisual();
		EnvCryRay.Render->model_Delete		(v);
	...
	m_model						= smart_cast<IKinematicsAnimated*>(EnvCryRay.Render->model_Create(model_name.c_str()));
	m_model_2					= smart_cast<IKinematicsAnimated*>(EnvCryRay.Render->model_Create(pSettings->line_exist(player_hud_sect, "visual_2") ? pSettings->r_string(player_hud_sect, "visual_2") : model_name.c_str()));
...
void player_hud::render_hud()
{
	...
	EnvCryRay.Render->set_Transform		(&m_transform);
	EnvCryRay.Render->add_Visual		(m_model->dcast_RenderVisual());

	EnvCryRay.Render->set_Transform		(&m_transform_2);
	EnvCryRay.Render->add_Visual		(m_model_2->dcast_RenderVisual());
