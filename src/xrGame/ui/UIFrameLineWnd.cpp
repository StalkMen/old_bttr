...
void draw_rect(Fvector2 LTp, Fvector2 RBp, Fvector2 LTt, Fvector2 RBt, u32 clr, Fvector2 const& ts)
{
	...
	EnvCryRay.UIRender->PushPoint(LTp.x, LTp.y,	0, clr, LTt.x, LTt.y);
	EnvCryRay.UIRender->PushPoint(RBp.x, RBp.y,	0, clr, RBt.x, RBt.y);
	EnvCryRay.UIRender->PushPoint(LTp.x, RBp.y,	0, clr, LTt.x, RBt.y);

	EnvCryRay.UIRender->PushPoint(LTp.x, LTp.y,	0, clr, LTt.x, LTt.y);
	EnvCryRay.UIRender->PushPoint(RBp.x, LTp.y,	0, clr, RBt.x, LTt.y);
	EnvCryRay.UIRender->PushPoint(RBp.x, RBp.y,	0, clr, RBt.x, RBt.y);
}
...
void CUIFrameLineWnd::DrawElements()
{
	...
	EnvCryRay.UIRender->StartPrimitive	(prim_count, IUIRender::ptTriList, UI().m_currentPointType);
