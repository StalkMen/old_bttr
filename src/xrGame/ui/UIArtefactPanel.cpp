...
void CUIArtefactPanel::InitIcons(const xr_vector<const CArtefact*>& artefacts)
{
    ...
    for (const CArtefact* artefact : artefacts)
    {
        ...
        Frect texture_rect;
        texture_rect.x1 = pSettings->r_float(sectionName, "inv_grid_x") * INV_GRID_WIDTH;
        texture_rect.y1 = pSettings->r_float(sectionName, "inv_grid_y") * INV_GRID_HEIGHT;
        texture_rect.x2 = pSettings->r_float(sectionName, "inv_grid_width") * INV_GRID_WIDTH;
        texture_rect.y2 = pSettings->r_float(sectionName, "inv_grid_height") * INV_GRID_HEIGHT;
