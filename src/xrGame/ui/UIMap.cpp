...
#pragma TODO("OldSerpskiStalker. Vanilla Call Of Chernobyl")
	...
	if (mouse_action == WINDOW_LBUTTON_DB_CLICK && (BttR_mode))
	{
...
void  CUIMiniMap::Draw()
{
	...
	EnvCryRay.UIRender->SetShader			(*m_UIStaticItem.GetShader());
	EnvCryRay.UIRender->StartPrimitive	(segments_count*3, IUIRender::ptTriList, UI().m_currentPointType);
	...
	{
		EnvCryRay.UIRender->PushPoint(S[0+0].pt.x,	S[0+0].pt.y,	0, color, S[0+0].uv.x,		S[0+0].uv.y);
		EnvCryRay.UIRender->PushPoint(S[idx+2].pt.x,	S[idx+2].pt.y,	0, color, S[idx+2].uv.x,	S[idx+2].uv.y);
		EnvCryRay.UIRender->PushPoint(S[idx+1].pt.x,	S[idx+1].pt.y,	0, color, S[idx+1].uv.x,	S[idx+1].uv.y);
	}
