...
#define tx 0.25f
	...
	EnvCryRay.UIRender->StartPrimitive	(max_prim_cnt, IUIRender::ptTriList, UI().m_currentPointType);
	...
	for ( int x = 0; x <= tgt_cells.width(); ++x )
	{
		for ( int y = 0; y <= tgt_cells.height(); ++y/*, ++cell_i*/ )
		{
			...
			for ( u32 k = 0; k < 6; ++k )
			{
				...
				EnvCryRay.UIRender->PushPoint(iFloor( rect_offset.x + p.x*(f_len.x) )-0.5f,
