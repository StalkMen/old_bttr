...
#ifdef XRGAME_EXPORTS
	return						(
		(flags.test(flR1) && (EnvCryRay.Render->get_generation() == IRender_interface::GENERATION_R1)) ||
		(flags.test(flR2) && (EnvCryRay.Render->get_generation() == IRender_interface::GENERATION_R2))
	);
#else
	return						(true);
#endif
