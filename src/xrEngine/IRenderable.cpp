...
{
    VERIFY(!g_bRendering);
    EnvCryRay.Render->model_Delete(renderable.visual);
    if (renderable.pROS) EnvCryRay.Render->ros_destroy(renderable.pROS);
    renderable.visual = NULL;
    renderable.pROS = NULL;
}
...
{
    if (0 == renderable.pROS && renderable.pROS_Allowed) renderable.pROS = EnvCryRay.Render->ros_create(this);
