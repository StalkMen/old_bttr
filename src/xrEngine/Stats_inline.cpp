    ...
    if (psDeviceFlags.test(rsRenderInfo))
    {
        ...
        for (size_t i = 0; i < LoopScale; i++)
        {
            switch (i)
            {
                ...
                case 3:
                        pFontHW->SetColor(DebugTextColor::DTC_TEST_DX);
                        pFontHW->Out(GetMainInfoStats, InfoScale, "RENDER TOKEN: %u", renderer_value);
                        InfoScale += 15;
                ...
                case 32: // Всегда должен быть последним параметром
