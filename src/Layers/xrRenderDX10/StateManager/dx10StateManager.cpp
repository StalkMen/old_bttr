...
void dx10StateManager::Apply()
{
	...
	if ( m_bRSNeedApply || m_bRSChanged )
	{
		...
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->RSSetState(m_pRState);
	...
	if ( m_bDSSNeedApply || m_bDSSChanged )
	{
		...
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->OMSetDepthStencilState(m_pDepthStencilState, m_uiStencilRef);
	...
	if ( m_bBSNeedApply || m_bBSChanged )
	{
		...
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->OMSetBlendState(m_pBlendState, BlendFactor, m_uiSampleMask);
