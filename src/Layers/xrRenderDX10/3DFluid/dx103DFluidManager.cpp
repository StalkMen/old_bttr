...
void dx103DFluidManager::CreateRTTextureAndViews(int rtIndex, D3D_TEXTURE3D_DESC TexDesc)
{
	...
	// Create the texture
	CHK_DX( DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateTexture3D(&TexDesc,NULL,&pRT));
	...
	CHK_DX( DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateRenderTargetView( pRT, &DescRT, &pRenderTargetViews[rtIndex]) );
...
void dx103DFluidManager::Reset()
{
	...
	for(int rtIndex=0; rtIndex<NUM_OWN_RENDER_TARGETS; rtIndex++)
	{
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( pRenderTargetViews[rtIndex], color );
	}
}
...
void dx103DFluidManager::Update( dx103DFluidData &FluidData, float timestep )
{
	...
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->RSSetViewports(1,&rtViewport);
...
void dx103DFluidManager::AdvectColorBFECC( float timestep, bool bTeperature )
{
	...
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( pRenderTargetViews[RENDER_TARGET_TEMPVECTOR], color );
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( pRenderTargetViews[RENDER_TARGET_TEMPSCALAR], color );
...
void dx103DFluidManager::UpdateObstacles( const dx103DFluidData &FluidData, float timestep )
{
	...
	//	Reset data
	float color[4] = {0, 0, 0, 0 };
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( pRenderTargetViews[RENDER_TARGET_OBSTACLES], color );
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( pRenderTargetViews[RENDER_TARGET_OBSTVELOCITY], color );
