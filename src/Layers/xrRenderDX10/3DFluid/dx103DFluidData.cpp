...
{
	// Create the texture
	CHK_DX( HW.pRenderDevice->CreateTexture3D(&TexDesc,NULL,&m_pRTTextures[rtIndex]));
	...
	CHK_DX( HW.pRenderDevice->CreateRenderTargetView( m_pRTTextures[rtIndex], &DescRT, &m_pRenderTargetViews[rtIndex]) );

	float color[4] = {0, 0, 0, 0 };

	HW.pRenderContext->ClearRenderTargetView( m_pRenderTargetViews[rtIndex], color );
}
