	...
	if (pSurface)
	{
        ...
        // if( ViewDesc.Format != DXGI_FORMAT_R24_UNORM_X8_TYPELESS )
				if( (desc.SampleDesc.Count <= 1) || (ViewDesc.Format != DXGI_FORMAT_R24_UNORM_X8_TYPELESS) )         
					CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateShaderResourceView(pSurface, &ViewDesc, &m_pSRView));
        else
           m_pSRView = 0;
		}
		else
			CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateShaderResourceView(pSurface, NULL, &m_pSRView));
	}	
}
...
void CTexture::ProcessStaging()
{
	...
	case D3D_RESOURCE_DIMENSION_TEXTURE2D:
		{
			...
			CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateTexture2D( &TexDesc,       // Texture desc
				NULL,                  // Initial data
				&T )); // [out] Texture
	...
	case D3D_RESOURCE_DIMENSION_TEXTURE3D:
		{
			...
			CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateTexture3D( &TexDesc,       // Texture desc
				NULL,                  // Initial data
				&T )); // [out] Texture
	...
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CopyResource(pTargetSurface, pSurface);
	/*
	for( int i=0; i<iNumSubresources; ++i)
	{
		DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CopySubresourceRegion(
			pTargetSurface,
			i,
			0,
			0,
...
void CTexture::Apply(u32 dwStage)
{
   ...
   //}
	...
	{
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->PSSetShaderResources(dwStage, 1, &m_pSRView);
		SRVSManager.SetPSResource(dwStage, m_pSRView);
	}
	else if (dwStage<rstGeometry)	//	Vertex shader stage resources
	{
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->VSSetShaderResources(dwStage-rstVertex, 1, &m_pSRView);
		SRVSManager.SetVSResource(dwStage-rstVertex, m_pSRView);
	}
	else if (dwStage<rstHull)	//	Geometry shader stage resources
	{
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->GSSetShaderResources(dwStage-rstGeometry, 1, &m_pSRView);
...
#ifdef DIRECTX11
		R_CHK				(DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->Map(T2D, 0, D3D_MAP_WRITE_DISCARD, 0, &mapData));
#else
...
#ifdef DIRECTX11
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->Unmap(T2D, 0);
#else
		T2D->Unmap(0);
#endif
	}
	Apply(dwStage);
	//CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetTexture(dwStage,pSurface));
};
...
//			HRESULT hrr = DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CreateTexture(
//				_w, _h, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &pTexture, NULL );
			...
			desc.MiscFlags = 0;
			HRESULT hrr = DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateTexture2D(&desc, 0, &pTexture);
			...
			{
				CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateShaderResourceView(pSurface, 0, &m_pSRView));
			}
				...
				while (!_fs->eof())
				{
					...
					if (buffer[0])	
					{
						...
						if (pSurface)	
						{
							...
							DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CreateShaderResourceView(seqDATA.back(), NULL, & m_seqSRView.back());
