...
void				TW_Save(ID3DTexture2D* T, LPCSTR name, LPCSTR prefix, LPCSTR postfix)
{
	...
	Log("! DX10 debug texture save: ", fn2);
...
CANT_LOAD:
	{
		Msg("! DX10 Can't find texture '%s'", fname);
	}
	const bool dummyTextureExist = FS.exist(fn, "$game_textures$", NOT_EXISTING_TEXTURE, ".dds");
#pragma todo("OldSerpskiStalker: Fix crash dx10Textures")
	R_ASSERT3(dummyTextureExist, "! [DX10]: Fake texture does not exist, everything is normal. The render continues...", NOT_EXISTING_TEXTURE);
...
_DDS:
	{
...
else
		{
			...
			{
				if (ps_r__common_flags.test(RFLAGDX_ENABLE_DEBUG_LOG))
					Msg("- [Debug]: Can't load texture DX10:%s", fn);
				goto CANT_LOAD;
			}
			else
			{
				...
				{
					if (ps_r__common_flags.test(RFLAGDX_ENABLE_DEBUG_LOG))
						Msg("- [Debug]: Cube texture DX10:%s", fn);
					goto _DDS_CUBE;
				}
				else
				{
	...
	_DDS_CUBE:
		{
			...
			if (ps_r__common_flags.test(RFLAGDX_OLD_LOAD_DDS))
			{
				...
				R_CHK(D3DX10CreateTextureFromMemory(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice, S->pointer(), S->length(), &LoadInfo, 0, &pTexture2D, 0));
				FS.r_close(S);
...
#pragma TODO("OldSerpskiStalker, по совету Zagolski")
				...
				R_CHK2(D3DX10CreateTextureFromMemory(DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice, S->pointer(), S->length(), &LoadInfo, 0, &pTexture2D, 0), fn);
				FS.r_close(S);
				mip_cnt = IMG.MipLevels;
				// OK
				ret_msize = calc_texture_size(img_loaded_lod, mip_cnt, img_size);
...
_BUMP_from_base:
	{
		...
		Msg("! Fallback to default bump map DX10: %s", fname);
		//////////////////
		if (strstr(fname, "_bump#"))
		{
			R_ASSERT2(FS.exist(fn, "$game_textures$", "ed\\ed_dummy_bump#", ".dds"), "ed_dummy_bump#");
