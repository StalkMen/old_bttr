...
void CRenderTarget::DoAsyncScreenshot()
{
	...
	if (RImplementation.m_bMakeAsyncSS)
	{
		...
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CopyResource( t_ss_async, pTex );
		ID3DTexture2D*	pBuffer;
		hr = DEVICE_HW::CRYRAY_RENDER::HW.m_pSwapChain->GetBuffer( 0, __uuidof( ID3DTexture2D ), (LPVOID*)&pBuffer );
#ifdef DIRECTX11
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CopyResource( t_ss_async, pBuffer );
#else
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->CopyResource( t_ss_async, pBuffer );
#endif
...
void	CRenderTarget::phase_combine	()
{
	...
	//*** exposure-pipeline
	u32			gpu_id	= Device.dwFrame%DEVICE_HW::CRYRAY_RENDER::HW.Caps.iGPUNum;
	if (Device.m_SecondViewport.IsSVPActive()) //--#SM+#-- +SecondVP+
		// clang-format off
		gpu_id = (Device.dwFrame - 1) % DEVICE_HW::CRYRAY_RENDER::HW.Caps.iGPUNum;
	...
	if( !RImplementation.o.dx10_msaa )
	{
		...
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView(rt_Generic_1->pRT, ColorRGBA);
#endif
	...
	{
#ifdef DIRECTX11
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView(rt_Generic_0_r->pRT, ColorRGBA);
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView(rt_Generic_1_r->pRT, ColorRGBA);
#else
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->ClearRenderTargetView(rt_Generic_0_r->pRT, ColorRGBA);
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->ClearRenderTargetView(rt_Generic_1_r->pRT, ColorRGBA);
#endif
...
//.		Fvector4	envclr			= { envdesc.sky_color.x*2+EPS,	envdesc.sky_color.y*2+EPS,	envdesc.sky_color.z*2+EPS,	envdesc.weight					};
...
#ifdef DIRECTX11	  
      DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ResolveSubresource( rt_Generic_1->pTexture->surface_get(), 0, rt_Generic_1_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );
      DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ResolveSubresource( rt_Generic_0->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );
#else
	  DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->ResolveSubresource( rt_Generic_1->pTexture->surface_get(), 0, rt_Generic_1_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );
      DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->ResolveSubresource( rt_Generic_0->pTexture->surface_get(), 0, rt_Generic_0_r->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM );	
#endif
   }
   ...
   // for msaa we need a resolved color buffer - Holger
	...
	//u_setrt(rt_Generic_1,0,0,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB);
	...
	BOOL	bDistort	= RImplementation.o.distortion_enabled;				// This can be modified
	{
		...
		if (bDistort)		
		{
			...
			if( !RImplementation.o.dx10_msaa )
			{
				...
				DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->ClearRenderTargetView( rt_Generic_1->pRT, ColorRGBA);
#else
				DEVICE_HW::CRYRAY_RENDER::HW.pRenderDevice->ClearRenderTargetView( rt_Generic_1->pRT, ColorRGBA);
#endif
			}
			else
			{
			...
			RCache.set_Stencil			(FALSE);
			RCache.set_ColorWriteEnable	();
			//CHK_DX(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->Clear	( 0L, NULL, D3DCLEAR_TARGET, color_rgba(127,127,0,127), 1.0f, 0L));
...
#pragma TODO("OldSerpskiStalker. Disabled. Because the rays pass through the materials of the location to the direction of the sun.")
	...
	//Gasmask
	if (mask_helmet) //Niech chuj jebie te maski w dupę pierdolone zabugowane gówno lvutnerowskie aaaaaaaaaaaaaaaaa
		phase_gasmask();
   ...
   {
      if		(PP_Complex)	u_setrt		( rt_Color,0,0,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB );			// LDR RT
      else					   u_setrt		( Device.dwWidth,Device.dwHeight,DEVICE_HW::CRYRAY_RENDER::HW.pBaseRT,NULL,NULL,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB);
   }
	//. u_setrt				( Device.dwWidth,Device.dwHeight,DEVICE_HW::CRYRAY_RENDER::HW.pBaseRT,NULL,NULL,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB);
	RCache.set_CullMode		( CULL_NONE )	;
