...
//		if (u_DBT_enable(zMin,zMax))	{
			// z-test always
//			DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState(D3DRS_ZFUNC, D3DCMP_ALWAYS);
//			DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
//		}
...
// Select target
	...
	phase_accumulator					();
	if( !RMSAA._opt.dx10_msaa )
		u_setrt								(rt_Generic_0,NULL,NULL,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB);
	else
	...
	if (SE_SUN_NEAR==sub_phase)	//.
	{
		...
		//CHK_DX	(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->Clear	( 0L, NULL, D3DCLEAR_TARGET, 0, 1.0f, 0L));
	...
	{
		if( !RMSAA._opt.dx10_msaa )
			u_setrt								(rt_Generic_0,NULL,NULL,DEVICE_HW::CRYRAY_RENDER::HW.pBaseZB);  // enshure RT setup
		else
...
void CRenderTarget::accum_direct_volumetric	(u32 sub_phase, const u32 Offset, const Fmatrix &mShadow)
{
	...
	if (!need_to_render_sunshafts() || ps_sunshafts_mode != SS_RENDER_ON)
		return;
...
//	TODO: DX10: Implement via different passes
			if (SE_SUN_NEAR==sub_phase)
				//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState( D3DRS_ZFUNC, D3DCMP_GREATER);
				RCache.set_ZFunc(D3DCMP_GREATER);
			else
				//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState( D3DRS_ZFUNC, D3DCMP_LESSEQUAL);
				RCache.set_ZFunc(D3DCMP_ALWAYS);
		}
