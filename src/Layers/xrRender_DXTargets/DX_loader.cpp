...
void CRender::level_Unload()
{
	...
	{
#ifdef DIRECTX10
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_vb	( nVB[I] );
#endif
		_RELEASE(nVB[I]);
	}
	...
	{
#ifdef DIRECTX10
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_vb ( xVB[I] );
#endif
		_RELEASE(xVB[I]);
	}
	nVB.clear(); xVB.clear();
	...
	{
#ifdef DIRECTX10
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_ib ( nIB[I] );
#endif
		_RELEASE(nIB[I]);
	}
	...
	{
#ifdef DIRECTX10
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_ib ( xIB[I] );
#endif
		_RELEASE(xIB[I]);
	}
...
//			D3DVERTEXELEMENT9*	dcl		= (D3DVERTEXELEMENT9*) fs().pointer();
			...
			// Create and fill
			//BYTE*	pData		= 0;
			//R_CHK				(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CreateVertexBuffer(vCount*vSize,dwUsage,0,D3DPOOL_MANAGED,&_VB[i],0));
...
#ifdef DIRECTX10
			DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.increment_stats_vb	(_VB[i]);	
#endif			
			xr_free(pData);
...
//			fs->advance			(vCount*vSize);
	...
	// Index buffers
	{
		...
		for (u32 i=0; i<count; i++)
		{
			...
			// Create and fill
			//BYTE*	pData		= 0;
			//R_CHK				(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CreateIndexBuffer(iCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&_IB[i],0));
...
#ifdef DIRECTX10
			DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.increment_stats_ib	(_IB[i]);
#endif												
			xr_free(pData);
