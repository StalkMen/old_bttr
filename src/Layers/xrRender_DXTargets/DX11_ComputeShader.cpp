...
void ComputeShader::Dispatch(u32 dimx, u32 dimy, u32 dimz)
{
	...
	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CSSetConstantBuffers(0, count, tempBuffer);

	if (!m_Textures.empty())
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CSSetShaderResources(0, m_Textures.size(), &m_Textures[0]);

	if (!m_Samplers.empty())
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CSSetSamplers(0, m_Samplers.size(), &m_Samplers[0]);

	if (!m_Outputs.empty())
	{
		UINT num = 0;
		DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->CSSetUnorderedAccessViews(0, m_Outputs.size(), &m_Outputs[0], &num);
	}

	DEVICE_HW::CRYRAY_RENDER::HW.pRenderContext->Dispatch(dimx, dimy, dimz);
}
