...
#pragma todo("OldSerpskiStalker. Новые дефайны для шейдеров")
	const u32 DX11		= renderer_value;
	const u32 DX10_1	= renderer_value;
	const u32 DX10		= renderer_value;
...
#ifdef USE_DX11
	strconcat(sizeof(folder), folder, "r4\\objects\\DX11\\", name, ".", extension);
#else
	strconcat(sizeof(folder), folder, "r3\\objects\\DX10\\", name, ".", extension);
#endif
