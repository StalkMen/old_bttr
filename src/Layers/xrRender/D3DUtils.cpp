...
//	VERIFY(!"SPrimitiveBuffer::CreateFromData not implemented for dx10");
	...
	R_CHK(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CreateVertexBuffer(v_cnt*stride, D3DUSAGE_WRITEONLY, 0, D3DPOOL_MANAGED, &pVB, 0));
	...
	R_CHK				(pVB->Unlock());
	if (i_cnt){ 
		R_CHK(DEVICE_HW::CRYRAY_RENDER::HW.pDevice->CreateIndexBuffer	(i_cnt*sizeof(u16),D3DUSAGE_WRITEONLY,D3DFMT_INDEX16,D3DPOOL_MANAGED,&pIB,NULL));
...
{                       
	if (pGeom){
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_vb	(pGeom->vb);
		DEVICE_HW::CRYRAY_RENDER::HW.stats_manager.decrement_stats_ib	(pGeom->ib);
		_RELEASE		(pGeom->vb);
		_RELEASE		(pGeom->ib);
		pGeom.destroy	();
	}
