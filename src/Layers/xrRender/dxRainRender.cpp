...
#pragma todo("qweasdd to himself: replace 40, 30,20 with sonsole command")
	...
	// Render if needed
	if (vCount)	{
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState	(D3DRS_CULLMODE,D3DCULL_NONE);
		RCache.set_CullMode(CULL_NONE);
		RCache.set_xform_world		(Fidentity);
		RCache.set_Shader			(SH_Rain);
		RCache.set_Geometry			(hGeom_Rain);
		RCache.Render				(D3DPT_TRIANGLELIST,vOffset,0,vCount,0,vCount/2);
		//DEVICE_HW::CRYRAY_RENDER::HW.pDevice->SetRenderState	(D3DRS_CULLMODE,D3DCULL_CCW);
