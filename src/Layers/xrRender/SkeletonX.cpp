...
{	
	...
	//u16			hw_bones_cnt		= u16((DEVICE_HW::CRYRAY_RENDER::HW.Caps.geometry.dwRegisters-22)/3);
	//	Igor: some shaders in r1 need more free constant registers
	u16			hw_bones_cnt		= u16((DEVICE_HW::CRYRAY_RENDER::HW.Caps.geometry.dwRegisters-22-3)/3);
	u16			sw_bones_cnt		= 0;
#ifdef _EDITOR
	hw_bones_cnt					= 0;
#endif
	...
	EnvCryRay.Render->shader_option_skinning	(-1);
	...
	case 1:
		{
			...
			if(1==bids.size())	
			{
				...
				EnvCryRay.Render->shader_option_skinning	(0);
			}else 
...
//.			R_ASSERT(sw_bones_cnt<=hw_bones_cnt);
			if(sw_bones_cnt<=hw_bones_cnt)
			{
				...
				RMS_bonecount					= sw_bones_cnt+1;
				EnvCryRay.Render->shader_option_skinning	(2);
			}
			else 
			{
				// software
...
//.			R_ASSERT(sw_bones_cnt<=hw_bones_cnt);
			if((sw_bones_cnt<=hw_bones_cnt))
			{
				...
				RMS_bonecount					= sw_bones_cnt+1;
				EnvCryRay.Render->shader_option_skinning	(3);
			}else
			{
				crc								= crc32	(data->pointer(),size);
...
//.			R_ASSERT(sw_bones_cnt<=hw_bones_cnt);
			if(sw_bones_cnt<=hw_bones_cnt)
			{
				...
				RMS_bonecount					= sw_bones_cnt+1;
				EnvCryRay.Render->shader_option_skinning	(4);
			}else
			{
				crc								= crc32	(data->pointer(),size);
